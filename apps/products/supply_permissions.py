...
def is_store_manager(user):
    ...
    return (
        user.is_superuser
        or (is_in_group(user, 'Cửa hàng trưởng')
        or is_in_group(user, 'Cua hang truong'))
    )
...
def is_warehouse_manager(user):
    ...
    return (
        user.is_superuser
        or (user.is_staff
        and (is_in_group(user, 'Quản lý kho')
        or is_in_group(user, 'Quan ly kho')))
    )
...
def can_view_inventory_check(request):
    """Quyền xem phiếu kiểm kê - CHỈ Quản lý kho và Admin."""
    user = request.user
